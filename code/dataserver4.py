...
#############################################
  ...
  def corrupt(self,key):
    ...
    self.data[key]=((Binary(pickle.dumps("This is a corrupted data4"))).data,end)
    return self.data[key]

  def terminate(self):
    print "data_server4 crashes"
    global quit
    quit = 1
    return 1

  def restart0(self):
    print "data_server4 restarts"
    self.data={}
    print "after restarting data_server4, its data will be:"
    print self.data
    return 1
#############################################    
...
if __name__ == "__main__":
    if len(argv) != 2:
        print 'usage: %s <dataserver4>. For example: python dataserver4.py 51244' % argv[0]
        exit(1)
    port = argv[1]
    main()
