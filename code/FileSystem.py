...
class FileNode:
    def __init__(self,name,isFile,path,port):
        self.name = name
        self.path = path
        self.port = "http://127.0.0.1:"+port
    ...
    def put(self,key,value):
        key = self.path+"&&"+key
        rpc = xmlrpclib.Server("http://127.0.0.1:"+port)
...
class FS:
    def __init__(self,port):
        self.port = "http://127.0.0.1:"+port
        self.root = FileNode('/',False,'/',"http://127.0.0.1:"+port)   #########################################
        now = time()
        self.fd = 0
    ...
    def add_dir(self,path,mode):
        # create a file node
        temp_node = FileNode(path.split('/')[-1],False,path,self.port)
    ...
    def add_file(self,path,mode):
        # create a file node
        temp_node = FileNode(path.split('/')[-1],True,path,self.port)
    ...
    def link_nodes(self,target,source):
        ...
        temp_node = FileNode(target.split('/')[-1],True,target,self.port)
...
class Memory(LoggingMixIn, Operations):
    ...
    def __init__(self,port):
        ...
        count +=1 # increment count for every method call, to track count of calls made.
        ...
        self.FS = FS("http://127.0.0.1:"+port)
...
if __name__ == "__main__":
  if len(argv) != 3:
    print 'usage: %s <mountpoint> <mediator port>. For example: python FileSystem.py fusemount 51234' % argv[0]
    exit(1)
  port = argv[2]
  # Create a new HtProxy object using the port specified at the command-line
  fuse = FUSE(Memory("http://127.0.0.1:"+port), argv[1], foreground=True)
