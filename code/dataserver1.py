...
#############################################
  ...
  def corrupt(self,key):
    ...
    self.data[key]=((Binary(pickle.dumps("This is a corrupted data1"))).data,end)
    return self.data[key]

  def terminate(self):
    print "data_server1 crashes"
    global quit
    quit = 1
    return 1

  def restart0(self):
    print "data_server1 restarts"
    self.data={}
    print "after restarting data_server1, its data will be:"
    print self.data
    return 1
#############################################    
...
if __name__ == "__main__":
    if len(argv) != 2:
        print 'usage: %s <dataserver1>. For example: python dataserver1.py 51241' % argv[0]
        exit(1)
    port = argv[1]
    main()
